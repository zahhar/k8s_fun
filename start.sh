...
ssh -t root@185.4.72.4 "export MASTER_IP=${master_ip} && export AZ_PWD=${az_pwd} && export ADM_PWD=PASSWORD; git clone git@github.com:zahhar/k8s_fun.git; sh ./k8s_fun/deploy.sh; /bin/bash -i"